...
int tx(uint32_t carrier_freq, int divider, char *audio_file, float ppm, float deviation, float mpx, char *control_pipe, int power, int gpio, int wait) {
	...
	for (;;) {
		...
		while (free_slots >= SUBSIZE) {
			...
			if(data_len == 0) {
				if(fm_mpx_get_samples(data, mpx, wait) < 0 ) {
					return 0;
				}
				data_len = DATA_SIZE;
				data_index = 0;
...
int main(int argc, char **argv) {
	...
	while((opt = getopt_long(argc, argv, short_opt, long_opt, NULL)) != -1)
	{
		switch(opt)
		{
			...
			case 'h': //help
				fatal("Help:\n"
				      "Syntax: pi_fm_exciter [--audio (-a) file] [--freq (-f) frequency] [--dev (-d) deviation] [--ppm (-p) ppm-error]\n"
				      "                  [--div (-D) divider] [--mpx (-m) mpx-power] [--power (-w) output-power] [--gpio (-g) gpio-pin]\n"
				      "			 [--wait (-W) wait-switch] [--ctl (-C) control-pipe]\n");
	...
	int errcode = tx(carrier_freq, best_divider, audio_file, ppm, deviation, mpx, control_pipe, power, gpio, wait);
